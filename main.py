...
def start(ticker):
    # BTCUSDT
    look_back = 11
    dataset = dataset_utils.Dataset(dataset_filename='data/'+ticker+'_d.csv')
    trades = dataset.get_full_trades(ticker)
    ...
    return {'exec-time': execution_time, 'ticker': ticker, 'granularity': 'day',
            'resource': './data/res.csv'}
